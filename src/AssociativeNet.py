...
class AssociativeNet(Model):
    ...
    def compute_weights(self):
        ...
        for p in range(K):
            ...
            weights_p_idx = []
            for q in xrange(K):
                #for i in xrange(len(self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V].data)):
                #    if self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V].data[i] < 2:
                #        self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V].data[i] = 0
                #    else:
                #        self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V].data[i] = 1
                #self.COUNTS.eliminate_zeros()

#                C_pq_rowS = self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V].sum(axis=0)
#                C_pq_colS = self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V].sum(axis=1)
#                denom = np.outer(np.sqrt(C_pq_rowS), np.sqrt(C_pq_colS))+0.000001
#                W_pq = self.COUNTS[p*V:(p+1)*V, q*V:(q+1)*V] /denom # C_ij/sqrt(Ci*Cj) )
...
#                W_pq = W_pq.todense()
#                ei, ev = eigs(W_pq)
#                W_pq.data = W_pq.data/280.0#iei[0].real
                #WrowS = W_pq.sum(axis=0)
                #WcolS = W_pq.sum(axis=1)
                #denom = np.outer(np.sqrt(WrowS), np.sqrt(WcolS)) + 100.0
                #W_pq = W_pq / denom

                #W_pq = W_pq.todense()
                #W_pq -= np.outer(ev[:, 0].real, ev[:, 0].real)
                #W_pq /= ei[0].real
    ...
    def ImplicitMatMul_localist(self, X, X0):
        ...
        for p in range(K):
            for q in range(K):
                for k in range(V):
                    for l in self.WEIGHTS_IDX[p][q][k]: #WEIGHT_IDX has the indices of nonzero cells for each row
                        Y[p*N + k] += beta*self.WEIGHTS[p][q][k, l]*X[q*N + l] #Mewmory term
                Y[p*N:(p+1)*N] += np.dot(X0[q*N:(q+1)*N], X[q*N:(q+1)*N])*X0[p*N:(p+1)*N] #STP term
        return Y
